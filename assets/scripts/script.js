...
function changeEventsByBall(id) {
	...
	imgPrincipal.src = events[Number.parseInt(position)].path;
...
function changeHost(index) {
	...
	hostImg.src = hosts[Number.parseInt(index)].path;
	hostTitle.innerHTML = hosts[Number.parseInt(index)].title;
	info.innerHTML = hosts[Number.parseInt(index)].info;
}
...
function centralizarNaSecaoAtual(divClicada) {
	// Obtém a seção atual
	const secaoAtual = divClicada.closest("section");

	// Verifica se a seção atual existe
	if (secaoAtual) {
		// Obtém a posição superior da seção atual em relação ao topo da página
		const posicaoSecao = secaoAtual.offsetTop;

		// Obtém a altura da barra de navegação (se houver)
		const alturaBarraNavegacao = document.querySelector("nav").offsetHeight || 0;
